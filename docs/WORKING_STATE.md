...
- The engine dashboard (mounted at `/mo_actions` in the dummy app) lists actions grouped by category with name + description. Argument-free actions get a one-click Run button; actions with arguments get a generated form (text/number/checkbox). Required fields are marked with `*`.
- Submitted arguments are validated on the action instance (ActiveModel) before cast/`perform`: blank required → presence error; non-integer for `:integer` → `not_a_number`. Invalid runs re-render the index at 422 with per-field errors and prior values, create no `Execution`, and do not call `perform`.
...
- Each dashboard run persists a `MoActions::Execution` (action key, coerced arguments as json, polymorphic performer when present, `succeeded`/`failed` status, optional `error_message`). The index shows the 20 most recent executions below the action list.
...
- `MoActions::Base` includes `ActiveModel::Model`. Instances keep raw submitted values until `#cast_arguments!` (after `#valid?`). Validation reads current `arguments` definitions so redeclare replaces cleanly.
- `ArgumentDefinition#cast` delegates to `ActiveModel::Type.lookup` (string/integer/boolean). Requiredness is stored on the definition (`required?`).
...
- Validation runs on raw input before cast — otherwise `ActiveModel::Type::Integer` would turn `"abc"` into `0`. Kept a single `validate_arguments` callback (not class-level `validates` per argument) so redeclare does not leave stale validators.
- Invalid runs stay on the actions index (422) rather than introducing a separate run page — enough for this slice; a dedicated form page can wait until drafts.
...
- `test/mo_actions/argument_dsl_test.rb` — declaration order, raw until cast, presence + integer checks, redeclare replaces.
- `test/mo_actions/execution_test.rb` — validations, recent scope, polymorphic performer, display-name fallback.
- `test/integration/dashboard_test.rb` — authenticated index; arg form fields; one-click Run; POST with args reaches `perform` coerced; unknown action 404s with no record; succeeded/failed execution persistence; recent executions list; blank required / bad integer → 422, no perform, no record, field errors + prior values.

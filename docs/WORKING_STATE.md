...
- Gem code lives in `lib/mo_actions/{base,registry,configuration,engine}.rb`; dashboard is one controller (`MoActions::ActionsController`) with `index` and `run`.
...
- `test/integration/dashboard_test.rb` — authenticated index lists actions grouped by category with run buttons; POST run invokes `perform` and flashes; unknown action 404s.

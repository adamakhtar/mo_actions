...
- `MoActions::Execution`: `STATUSES = %w[running succeeded failed]`, optional polymorphic `performer`, `progress_current` (default 0), optional `progress_total`, `recent` scope, `progress_percent`, `action_display_name` (registry lookup with key fallback).
...
- `test/integration/dashboard_test.rb` — actions index links; run page forms; create with coerced args; unknown key 404; succeeded/failed persistence; executions index + action_key filter; validation 422 on run page; execution show (progress, running refresh, failed/unregistered) + 404; demo backfill progress.
